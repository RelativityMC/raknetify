...
import static com.ishland.raknetify.fabric.common.connection.SynchronizationLayer.accessible;
...
//            channel.pipeline().addLast("raknetify-flush-consolidation", new FlushConsolidationHandler(Integer.MAX_VALUE, true));
            ...
            channel.pipeline().addLast("raknetify-streaming-compression", new MultiChannelingStreamingCompression(Constants.RAKNET_GAME_PACKET_ID, Constants.RAKNET_STREAMING_COMPRESSION_PACKET_ID));
            channel.pipeline().addLast("raknetify-multi-channel-data-codec", new MultiChannellingDataCodec(Constants.RAKNET_GAME_PACKET_ID));
